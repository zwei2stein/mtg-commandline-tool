...
class PriceSource:
    ...
    def flushCache(self):
        try:
            ...
            print("Error where clearing cache directory at " + self.getCacheDir() + ": %s - %s." % (
    ...
    def initCache(self, collection):
        ...
        for card in collection:
            ...
            statusLine = 'Fetching ' + self.sourceName + ' price for (' + str(count) + '/' + str(
                len(collection)) + ', ...' + str(collection[card].sourceFile)[-50:-2] + '): "' + card + '" ...'
    ...
    def getCardPrice(self, card):
        ...
        if (os.path.exists(jsonFile)):
            ...
            if (self.clearCache == 'always' or (self.clearCache == 'timeout' and fileAge.days > self.cacheTimeout) or (
                    self.clearCache == 'price' and fileAge.days > 1)):
